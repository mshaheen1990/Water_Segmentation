...
def tiled_predict(model, x, tile_size=64, stride=64):
    ...
    for y0 in range(0, ph - tile_size + 1, stride):
        for x0 in range(0, pw - tile_size + 1, stride):
            pred = model.predict(pad[y0:y0 + tile_size, x0:x0 + tile_size][None, ...], verbose=0)[0]
            prob[y0:y0 + tile_size, x0:x0 + tile_size] += pred
            cnt[y0:y0 + tile_size, x0:x0 + tile_size] += 1.0
    return (prob / np.maximum(cnt, 1.0))[:h, :w]


def run_tiled_evaluation(model, test_df, cfg, threshold=0.5, out_csv='results/tiled_full_image_test_results.csv'):
    rows = []
    for _, r in test_df.iterrows():
        x, y = build_full_input(r['planet_path'], r['hecras_path'], r['qmask_path'], cfg)
        p = tiled_predict(model, x, tile_size=cfg.image_size, stride=cfg.image_size)
    ...
    return out, summary.iloc[0].to_dict()
...
def main(config_path):
    ...
    out, summary = run_tiled_evaluation(model, test_df, dcfg, threshold=0.5)
    print(f"Saved tiled full-image results to {out} | mean_iou={summary['mean_iou']:.6f} mean_dice={summary['mean_dice']:.6f}")
